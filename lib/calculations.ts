import { daysLeftInPeriod, toDate } from "./budget";
import type { Bill, CardItem, CompanyExpenseItem, FinanceData, OtClaimItem, Transaction, WalletSnapshot } from "./types";
...
export function cardOutstanding(cardName: string, cardItems: CardItem[], transactions: Transaction[]): number {
  const cardItemCharges = cardItems.filter((item) => item.card === cardName && item.paid_status === "unpaid").reduce((sum, item) => sum + item.amount, 0);
  ...
  const payments = transactions
    .filter((transaction) => transaction.type === "card_payment" && transaction.card === cardName && transaction.budget_month === ACTIVE_BUDGET && isCashCleared(transaction))
    .reduce((sum, transaction) => sum + transaction.amount, 0);
